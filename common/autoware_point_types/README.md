...
`autoware_point_types` defines multiple structures (such as PointXYZI, PointXYZIRC, PointXYZIRCT, PointXYZIRADRT, PointXYZIRCAEDT), each structure contains different attributes to adapt to different application scenarios.
...
- `autoware::point_types::PointXYZIRCT`: Extended PointXYZIRC, adds time_stamp information. Standard format after preprocessing (20 bytes).
...
- Create a point cloud object of PointXYZIRCT type

```cpp
#include "autoware/point_types/types.hpp"

int main(){
    pcl::PointCloud<autoware::point_types::PointXYZIRCT>::Ptr cloud(new pcl::PointCloud<autoware::point_types::PointXYZIRCT>());

    for (int i = 0; i < 5; ++i) {
        autoware::point_types::PointXYZIRCT point;
...
- Convert ROS message to point cloud of PointXYZIRCT type

```cpp
ExampleNode::points_callback(const PointCloud2::ConstSharedPtr & points_msg_ptr)
{
    pcl::PointCloud<autoware::point_types::PointXYZIRCT>::Ptr points_ptr(
    new pcl::PointCloud<autoware::point_types::PointXYZIRCT>);
