...
public:
  void initialize(const std::string & /*name*/, rclcpp::Node * node, MapLoaderData & /*data*/) override
  {
    ...
    allow_unsupported_version_ =
      get_or_declare_parameter<bool>(*node, "allow_unsupported_version");
    ...
    pub_map_bin_ = node->create_publisher<VectorMap::Message>(
      VectorMap::name, rclcpp::QoS{1}.transient_local());
  }
  ...
  void on_startup(MapLoaderData & data) override
  {
    ...
    if (use_waypoints_) {
      lanelet::utils::overwriteLaneletsCenterlineWithWaypoints(
        map, center_line_resolution_, false);
    } else {
