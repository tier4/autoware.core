...
public:
  ...
  void on_startup(MapLoaderData & data) override
  {
    if (!data.vector_map_msg.has_value()) {
      RCLCPP_WARN(get_node_ptr()->get_logger(), "Vector map is not loaded; skipping TF generation.");
      return;
    }
    ...
    RCLCPP_INFO_STREAM(
      get_node_ptr()->get_logger(), "Broadcast static tf. map_frame:"
                                      << map_frame_ << ", viewer_frame:" << viewer_frame_);
  }
