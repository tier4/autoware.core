...
// ---------------------------------------------------------------------------
// ImplBase — common logic shared by rclcpp and agnocast implementations
// ---------------------------------------------------------------------------
struct MapHeightFitter::ImplBase
{
  ...
  virtual bool has_partial_loader() const = 0;
  virtual bool get_partial_point_cloud_map(const Point & point) = 0;

  void on_pcd_map_data(const PointCloud2 & msg)
  {
  ...
  void on_vector_map_data(const LaneletMapBin & msg)
  {
    vector_map_ = std::make_shared<lanelet::LaneletMap>();
    lanelet::utils::conversion::fromBinMsg(*const_cast<LaneletMapBin *>(&msg), vector_map_);
    map_frame_ = msg.header.frame_id;
  }
...
// ---------------------------------------------------------------------------
// RclcppImpl — rclcpp::Node specific implementation
// ---------------------------------------------------------------------------
struct RclcppImpl : MapHeightFitter::ImplBase
{
  ...
  rclcpp::Client<GetPartialPointCloudMap>::SharedPtr cli_pcd_map_;
  rclcpp::Subscription<PointCloud2>::SharedPtr sub_pcd_map_;
  rclcpp::AsyncParametersClient::SharedPtr params_pcd_map_loader_;
  rclcpp::Subscription<LaneletMapBin>::SharedPtr sub_vector_map_;

  explicit RclcppImpl(rclcpp::Node * node) : node_(node), tf2_listener_(tf2_buffer_)
  {
    ...
    if (fit_target_ == "pointcloud_map") {
      const auto callback =
        [this](const std::shared_future<std::vector<rclcpp::Parameter>> & future) {
          ...
          if (partial_load) {
            ...
            cli_pcd_map_ = node_->create_client<GetPartialPointCloudMap>(
              "~/partial_map_load", rmw_qos_profile_default, group_);
          } else {
            const auto durable_qos = rclcpp::QoS(1).transient_local();
            sub_pcd_map_ = node_->create_subscription<PointCloud2>(
              "~/pointcloud_map", durable_qos,
              [this](const PointCloud2::ConstSharedPtr msg) { on_pcd_map_data(*msg); });
          }
        };
      ...
      const auto durable_qos = rclcpp::QoS(1).transient_local();
      sub_vector_map_ = node_->create_subscription<LaneletMapBin>(
        "~/vector_map", durable_qos,
        [this](const LaneletMapBin::ConstSharedPtr msg) { on_vector_map_data(*msg); });
  ...
  rclcpp::Logger get_logger() const override { return node_->get_logger(); }
  bool has_partial_loader() const override { return !!cli_pcd_map_; }

  bool get_partial_point_cloud_map(const Point & point) override
  {
    const auto logger = get_logger();
    ...
    const auto req = std::make_shared<GetPartialPointCloudMap::Request>();
    ...
    for (const auto & pcd_with_id : res->new_pointcloud_with_ids) {
      if (pcd_msg.width == 0) {
        ...
        pcd_msg.data.insert(
          pcd_msg.data.end(), pcd_with_id.pointcloud.data.begin(),
...
// ---------------------------------------------------------------------------
// AgnocastImpl — agnocast::Node specific implementation
// ---------------------------------------------------------------------------
struct AgnocastImpl : MapHeightFitter::ImplBase
{
  agnocast::Node * node_;
  agnocast::TransformListener tf2_listener_;
  rclcpp::CallbackGroup::SharedPtr group_;
  agnocast::Client<GetPartialPointCloudMap>::SharedPtr cli_pcd_map_;
  agnocast::Subscription<PointCloud2>::SharedPtr sub_pcd_map_;
  agnocast::Subscription<LaneletMapBin>::SharedPtr sub_vector_map_;

  explicit AgnocastImpl(agnocast::Node * node)
  : node_(node), tf2_listener_(tf2_buffer_, *node)
  {
    fit_target_ = node->declare_parameter<std::string>("map_height_fitter.target");
    if (fit_target_ == "pointcloud_map") {
      const bool partial_load =
        node->declare_parameter<bool>("map_height_fitter.enable_partial_load", false);
      ...
      if (partial_load) {
        ...
        sub_pcd_map_ = node_->create_subscription<PointCloud2>(
          "~/pointcloud_map", durable_qos,
          [this](const agnocast::ipc_shared_ptr<const PointCloud2> & msg) {
            on_pcd_map_data(*msg);
          });
      }
      ...
      sub_vector_map_ = node_->create_subscription<LaneletMapBin>(
        "~/vector_map", durable_qos,
        [this](const agnocast::ipc_shared_ptr<const LaneletMapBin> & msg) {
          on_vector_map_data(*msg);
        });
  ...
  bool get_partial_point_cloud_map(const Point & point) override
  {
    ...
    auto req = cli_pcd_map_->borrow_loaned_request();
    req->area.center_x = static_cast<float>(point.x);
    req->area.center_y = static_cast<float>(point.y);
    req->area.radius = 50;

    RCLCPP_DEBUG(logger, "Send request to map_loader");
    const auto res = cli_pcd_map_->async_send_request(std::move(req)).get();
    RCLCPP_DEBUG(
      logger, "Loaded partial pcd map from map_loader (grid size: %lu)",
      res->new_pointcloud_with_ids.size());
...
// ---------------------------------------------------------------------------
// MapHeightFitter — public API
// ---------------------------------------------------------------------------
MapHeightFitter::MapHeightFitter(rclcpp::Node * node)
{
  impl_ = std::make_unique<RclcppImpl>(node);
}

MapHeightFitter::MapHeightFitter(agnocast::Node * node)
{
  impl_ = std::make_unique<AgnocastImpl>(node);
}
